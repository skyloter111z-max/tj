...
def main():
    ...
    print(f"\n감시 시작 ({args.every}초 간격, 레벨 ±{args.near}% 안이면 알림"
            ...
            for t in tickers:
                ...
                for name, lvl in levels[coin]:
                    ...
                    if crossed and state.get(key) != "hit":
                        state[key] = "hit"
                        notify(f"{coin} {name} 도달", f"{coin} {price:,.0f}원이 {name} {lvl:,.0f}원에 닿았습니다.")
                    elif abs(dist) <= args.near and key not in state:
                        state[key] = "near"
                        notify(f"{coin} {name} 근접", f"{coin} {price:,.0f}원, {name} {lvl:,.0f}원까지 {(lvl / price - 1) * 100:+.2f}%")
                    elif abs(dist) > args.near * 2 and key in state:
